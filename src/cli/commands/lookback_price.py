...
def setup_parser(subparsers):
    parser = subparsers.add_parser("lookback-price", help="Lookback option pricing (GBM or Heston Monte Carlo)")
    add_standard_market_args(parser, require_sigma=False)
    parser.add_argument("--lookback-style", choices=["fixed", "floating"], default="fixed", help="Fixed-strike or floating-strike lookback")
...
def run(args):
    wf = PricingWorkflow("lookback-price", args)
    wf.run_lookback(args)
